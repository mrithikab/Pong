...
def update_game_state(dto):
    ...
    This method also resets the position if ball hits any wall.
    ...
    # Check if the ball is near the left wall
        ...
        if dto.player_y[0] <= game_dto.ball_y <= (dto.player_y[0] + bat_height):
            game_dto.hit=True #to play hit sound
        ...
        # Check if the ball is not hitting the left wall [CHANGED]
        elif not (game_dto.ball_x < ((ball_diameter / 2) + game_dto.ball_velocity_x) \
            and game_dto.ball_direction_x == 'negative'):
            game_dto.ball_x -= game_dto.ball_velocity_x
    ...
    # Check if the ball is near the right wall
        ...
        if dto.player_y[1] <= game_dto.ball_y <= (dto.player_y[1] + bat_height):
            game_dto.hit=1 #to play hit sound
            # Based on which part of the bat (4 parts), the ball has hit, the vertical direction of ball is updated.
        ...
        elif not (game_dto.ball_x > (window_width - (ball_diameter / 2) - game_dto.ball_velocity_x) \
            and game_dto.ball_direction_x == 'positive'):
            game_dto.ball_x += game_dto.ball_velocity_x
...
def get_game_player_id():
    ...
    for game in game_ids:
        # If the game has only one player waiting, assign the new player to this game
        if len(game.player_ids) == 1:
            ...
            # Since the game now has both players, indicate the start of play
            game.game_dto.start_play = True
            return game.game_id, player_id  # Return the current game_id and the new player_id

    # If no game was found with exactly one player, create a new game
    if not found:
        # Increment the last game id by 1 to create the new game id (if there are games in queue)
        if len(game_ids) > 0:
            game_id = game_ids[-1].game_id + 1
        else:
            game_id = 0  # This is the first game if no games exist yet

        player_id = 0  # First player in a new game always gets player_id 0
        game = Game()
    ...
    while run:
        ...
        try:
            ...
            if not receive_dto:
                ...
                if receive_dto.start_play: # Update game state e.g. ball movement, player’s movement and points etc.
